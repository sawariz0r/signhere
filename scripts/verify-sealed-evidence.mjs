...
export async function verifySealedEvidence(manifest, original, completed, uploaded, trustedFingerprint) {
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  const args = process.argv.slice(2), trustIndex = args.indexOf('--trust-fingerprint');
  ...
  try {
    ...
    if (args.length < 3 || args.length > 4 || (trustIndex >= 0 && !/^[a-f0-9]{64}$/.test(trustedFingerprint ?? ''))) throw new Error('Usage: node verify-sealed-evidence.mjs evidence.json original.pdf completed.pdf [uploaded.pdf] [--trust-fingerprint HEX]');
