...
The planned [central trust service](central-service-plan.md) (independent timestamps and email approval for participants without BankID) is likewise optional and not implemented. Installations need no central account or network access; with no central URL configured nothing is sent there. Installations using BankID or a comparable independent provider do not need central approval.
