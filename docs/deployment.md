...
The [central trust service](central-service-plan.md) is likewise optional. Its independent email approval for participants without BankID is available as a beta (set `SIGNHERE_CENTRAL_URL`, `SIGNHERE_CENTRAL_TRUST_ROOT` and `SIGNHERE_CENTRAL_API_KEY`; see [central-deployment.md](central-deployment.md)); independent timestamps are not implemented. Installations need no central account or network access; with no central URL configured nothing is sent there. Installations using BankID or a comparable independent provider do not need central approval.
