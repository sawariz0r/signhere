...
function App() {
  ...
  useEffect(() => { if (current.path.startsWith('/editor/')) return; document.title = `${sent ? 'Skickat' : current.path === '/sign' ? 'Signera' : current.path === '/verify' || current.path === '/verifiera' ? 'Verifiera dokument' : current.path === '/team' ? 'Team' : current.path === '/new' ? 'Nytt dokument' : current.path.endsWith('/bilaga') ? 'Ny bilaga' : 'Dokument'} · signhere`; }, [current.path, sent]);
  ...
  if (current.path.startsWith('/editor/')) {
    ...
    return <Suspense fallback={<main className="main"><Loading>Öppnar editorn…</Loading></main>}><DocumentEditor key={id} draftId={id} user={user} attachment={attachment} emailEnabled={Boolean(bootstrap.delivery?.email)} central={bootstrap.central} onSent={onSent} onClose={() => navigate(parentId ? `/documents/${parentId}/bilaga` : '/')} /></Suspense>;
  }
  ...
  return <><header className="app-header no-print"><div className="header-inner"><Brand publicBrand={!user} />{user ? <><nav aria-label="Huvudmeny">{[['docs', 'Dokument', '/'], ['team', 'Team', '/team'], ['verify', 'Verifiera', '/verify']].map(([key, label, path]) => <button key={key} className={activeNav === key ? 'active' : ''} aria-current={activeNav === key ? 'page' : undefined} onClick={() => navigate(path)}>{label}</button>)}</nav><button className="header-account" onClick={() => navigate('/team')} aria-label={`${user.name}, ${user.teamName}`}><span>{user.teamName}</span><Avatar name={user.name} dark /></button></> : <button className="button secondary small" onClick={() => navigate('/')}>{bootstrap.setupRequired ? 'Skapa konto' : 'Logga in'}</button>}</div></header><main className="main" id="main-content">
    {sent && documentId === sent.created.document.id ? <EditorSent key={documentId} result={sent} onOpen={id => navigate(`/documents/${id}`)} /> : user && current.path === '/team' ? <Team user={user} onRename={teamName => setBootstrap({ ...bootstrap, user: { ...user, teamName } })} onLogout={() => { setBootstrap({ ...bootstrap, user: null }); navigate('/'); }} /> : user && current.path === '/new' ? <NewDocument user={user} central={bootstrap.central} onCancel={() => navigate('/')} onOpen={id => navigate(`/documents/${id}`)} /> : documentId && newAttachment ? <NewAttachment key={documentId} parentId={documentId} user={user} central={bootstrap.central} onCancel={() => navigate(`/documents/${documentId}`)} onOpen={id => navigate(`/documents/${id}`)} onEditor={parent => navigate(`/editor/${draftId()}?bilaga=${parent.id}`)} /> : documentId ? certificate ? <Certificate doc={certificate} onBack={() => setCertificate(null)} onVerify={() => navigate('/verify')} /> : <DocumentDetail key={documentId} id={documentId} user={user} onBack={() => navigate('/')} onOpen={id => navigate(`/documents/${id}`)} onAddAttachment={() => navigate(`/documents/${documentId}/bilaga`)} onCertificate={doc => { setCertificate(doc); window.scrollTo(0, 0); }} /> : <Documents onOpen={id => navigate(`/documents/${id}`)} onNew={() => navigate('/new')} onCreate={() => navigate(`/editor/${draftId()}`)} onDraft={id => navigate(`/editor/${id}`)} />}
  </main></>;
}
