...
import type { Brand as BrandData, Dossier as DossierData, PartyDocument, PartyEvent, SignSession, SigningDocument, Strokes } from './types';
import { Brand, BrandLockup, ErrorBox, Field, Loading, PdfIcon, Signature } from './ui';
...
export function CompletedCopy({ token }: { token: string }) {
  ...
  return <div className="sign-screen" style={accentStyle(brand)}><header className="sign-header"><div>{brand ? <BrandLockup brand={brand} fallback="signhere" className="large" /> : !token && <Brand />}</div></header><main className="sign-done"><h1>Ditt signerade dokument</h1><p>Ladda ner din signerade PDF-fil.</p><ErrorBox error={error} /><button className="button" disabled={busy || !token} onClick={async () => { setBusy(true); setError(''); try { await download('/api/copy/download', 'signhere_signerat.pdf', { token }); } catch (error) { setError(message(error)); } finally { setBusy(false); } }}>{busy ? 'Hämtar…' : 'Ladda ner signerad PDF'}</button>{token && <Dossier token={token} readOnly onBrand={setBrand} />}</main></div>;
}
...
function Dossier({ token, currentId, onOpen, onBrand, readOnly = false, compact = false }: { token: string; currentId?: string; onOpen?: (documentId: string) => void; onBrand?: (brand: BrandData) => void; readOnly?: boolean; compact?: boolean }) {
  const [dossier, setDossier] = useState<DossierData | null>(null);
  const [error, setError] = useState('');
  const [busy, setBusy] = useState('');
  useEffect(() => { let active = true; void request<DossierData>('/api/sign/dossier', { token }).then(result => { if (active) { setDossier(result); onBrand?.(result.brand); } }).catch(error => { if (active) setError(message(error)); }); return () => { active = false; }; }, [token, currentId]);
