...
function SignatureModal({ session, token, hash, onClose, onComplete }: { session: SignSession; token: string; hash: string; onClose: () => void; onComplete: (doc: SigningDocument) => void }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try { const result = await request<{ document: SigningDocument }>('/api/sign/complete', { token, documentHash: hash, consentVersion: session.consent.version, accepted: true, name: name.trim(), payload: { strokes } }); onComplete(result.document); }
