...
import { Avatar, CopyLink, Dropzone, ErrorBox, Field, IndependentApprovalOption, Loading, PdfIcon, Signature, Status } from './ui';
import type { AuditEvent, Central, CreatedDocument, Delivery, PreparedPdf, SigningDocument, User } from './types';
...
export function NewDocument({ user, onCancel, onOpen, parent, onEditor, central }: { user: User; onCancel: () => void; onOpen: (id: string) => void; parent?: SigningDocument; onEditor?: () => void; central?: Central }) {
  ...
  const send = async (event: FormEvent) => {
    ...
    setBusy(true);
    try {
      const fields = { title: title.trim(), fileName: file.name, pdfBase64: prepared.rawBase64, recipients: selected, includeSender: includeMe && !coversMe, methodId: 'draw', ...(independent ? { independentApproval: true } : {}) };
      const result = parent
...
export function Certificate({ doc, onBack, onVerify }: { doc: SigningDocument; onBack: () => void; onVerify: () => void }) {
  ...
  return <div className="certificate-screen"><div className="actions between certificate-toolbar no-print"><button className="text-button" onClick={onBack}>← Tillbaka</button><div className="actions">{doc.status === 'completed' && <button className="button secondary small" onClick={() => { setError(''); void download(`/api/documents/${doc.id}/verification-package`, `${doc.id}_verifiering.zip`).catch(error => setError(message(error))); }}>Hämta verifieringspaket</button>}<button className="button secondary small" onClick={() => { setError(''); void download(`/api/documents/${doc.id}/evidence`, `${doc.id}_verifikat.json`).catch(error => setError(message(error))); }}>Hämta bevisdata</button><button className="button secondary small" onClick={() => window.print()}>Skriv ut / spara PDF</button></div></div><ErrorBox error={error} /><article className="certificate"><div className="certificate-rule" /><div className="certificate-title"><div><span className="mono muted text-small">signhere</span><h1>Verifikat</h1></div><div className="stamp"><div><strong>SIGNHERE</strong><i /><b>{doc.status === 'completed' ? 'SLUTFÖRT' : doc.status === 'cancelled' ? 'AVBRUTET' : 'PÅGÅR'}</b><span className="mono">{doc.completedAt ? new Date(doc.completedAt).toLocaleDateString('sv-SE') : '—'}</span></div></div></div><div className="certificate-dots" />
      ...
      <h2>Signerande parter</h2><div className="certificate-signers">{doc.recipients.map(recipient => <div className="certificate-signer" key={recipient.id}><strong>{recipient.signedName || recipient.name}</strong>{recipient.signedName && recipient.signedName !== recipient.name && <p className="muted text-tiny">Inbjuden som {recipient.name}</p>}<p>Undertecknare{recipient.email && ` · ${recipient.email}`}</p><div className="certificate-signature"><Signature strokes={recipient.signature?.strokes} /></div><b>{recipient.signedAt ? 'Undertecknad med ritad signatur' : 'Väntar på signatur'}</b>{recipient.independentApproval?.status === 'verified' && doc.independentApproval && <p>E-post bekräftad och dokumentet godkänt via {new URL(doc.independentApproval.service).host} (identitet ej verifierad)</p>}<p>Signerade {dateTime(recipient.signedAt)}</p>{recipient.ip && <p className="mono">IP {recipient.ip}</p>}{recipient.userAgent && <p className="device">{recipient.userAgent}</p>}{recipient.viewedAt && <p>Öppnade {dateTime(recipient.viewedAt)}</p>}</div>)}</div>
...
export function NewAttachment({ parentId, user, onCancel, onOpen, onEditor, central }: { parentId: string; user: User; onCancel: () => void; onOpen: (id: string) => void; onEditor: (parent: SigningDocument) => void; central?: Central }) {
  ...
  return <NewDocument user={user} parent={parent} onCancel={onCancel} onOpen={onOpen} onEditor={() => onEditor(parent)} central={central} />;
}
