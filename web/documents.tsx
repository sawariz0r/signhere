...
import { date, dateTime, download, fetchPdf, fileBase64, message, pages, request, size } from './api';
...
import type { AuditEvent, CreatedDocument, PreparedPdf, SigningDocument, User } from './types';
...
export function Documents({ onOpen, onNew, onCreate, onDraft }: { onOpen: (id: string) => void; onNew: () => void; onCreate: () => void; onDraft: (id: string) => void }) {
  ...
  useEffect(load, []);
  const visible = documents?.filter(doc => filter === 'all' || doc.status === filter) ?? [];
  return <div><div className="page-heading"><h1>Dokument</h1><div className="actions"><button className="button secondary" onClick={onCreate}>Skapa i editorn</button><button className="button" onClick={onNew}><span className="plus">+</span>Nytt dokument</button></div></div>
    ...
    {documents && (documents.length === 0 ? <button className="empty-state" onClick={onNew}><span className="circle">+</span><strong>Skicka ditt första avtal</strong><span>Ladda upp en PDF så är du igång.</span></button> : <>
      ...
      <div className="document-list">{visible.map(doc => <button className="document-row" key={doc.id} onClick={() => onOpen(doc.id)}><PdfIcon /><span className="document-name"><strong>{doc.title}</strong><span>Till {doc.recipients.map(r => r.name).join(', ')}</span></span><Status doc={doc} /><span className="document-date">{date(doc.createdAt)}</span></button>)}{!visible.length && <div className="loading">Inga dokument här.</div>}</div>
    </>)}
  </div>;
}
...
export function NewDocument({ user, onCancel, onOpen }: { user: User; onCancel: () => void; onOpen: (id: string) => void }) {
  ...
  const signerCount = recipientCount + (includeMe ? 1 : 0);
  ...
  const send = async (event: FormEvent) => {
    ...
    if (!selected.length && !includeMe) { setError('Lägg till minst en mottagare.'); return; }
    ...
    setBusy(true);
    try {
      const result = await request<CreatedDocument>('/api/documents', { title: title.trim(), fileName: file.name, pdfBase64: prepared.rawBase64, recipients: selected, includeSender: includeMe, methodId: 'draw' });
      setCreated(result); setStep(2); clearFile();
      const senderAssignmentValid = !includeMe || hasExpectedSenderAssignment(result, user, selected);
      ...
      setSenderSigning(includeMe && senderAssignmentValid); window.scrollTo(0, 0);
  ...
  return <div className="narrow"><div className="page-heading new-heading"><h1>Nytt dokument</h1><button className="text-button" disabled={busy} onClick={() => created ? onOpen(created.document.id) : onCancel()}>{created ? 'Stäng' : 'Avbryt'}</button></div>
    <div className="steps" aria-label={`Steg ${step + 1} av 3`}>{['Dokument', 'Mottagare', senderPending ? 'Signera' : created?.document.status === 'completed' ? 'Klart' : 'Dela'].map((label, i) => <div className={`step${i <= step ? ' active' : ''}`} key={label}><span>{i < step ? '✓' : i + 1}</span><strong>{label}</strong><i /></div>)}</div>
    <section className="card new-card"><ErrorBox error={error} />
      ...
      </form> : <><Dropzone onFile={value => void handleFile(value)} title="Släpp din PDF här" subtitle="eller klicka för att välja fil" /><p className="upload-note">PDF · högst 10 MB och 100 sidor</p></>)}
      {step === 1 && <form className="stack" onSubmit={send}><div><h2>Vem ska signera?</h2><p className="muted text-small">Alla signerar samtidigt.</p></div><div className="stack small-gap">{recipients.map((recipient, i) => <div className="recipient-inputs" key={i}><input aria-label={`Mottagare ${i + 1}, namn`} placeholder="Namn" value={recipient.name} maxLength={160} onChange={e => setRecipients(recipients.map((r, j) => j === i ? { ...r, name: e.target.value } : r))} /><input aria-label={`Mottagare ${i + 1}, e-post`} placeholder="E-post" type="email" value={recipient.email} maxLength={254} onChange={e => setRecipients(recipients.map((r, j) => j === i ? { ...r, email: e.target.value } : r))} /><button className="icon-button" type="button" aria-label={`Ta bort mottagare ${i + 1}`} onClick={() => setRecipients(recipients.filter((_, j) => i !== j))}>×</button></div>)}</div><div className="actions between"><button type="button" className="button secondary small" disabled={recipients.length >= 20} onClick={() => setRecipients([...recipients, { name: '', email: '' }])}>+ Lägg till mottagare</button><label className="checkbox"><input type="checkbox" checked={includeMe} onChange={e => setIncludeMe(e.target.checked)} />Jag ska också signera</label></div><div className="stack small-gap" aria-live="polite"><p className="text-small">{includeMe ? `Du (${user.name})${recipientCount ? ` + ${recipientCount} mottagare` : ''}` : `${recipientCount} mottagare`} · {signerCount} {signerCount === 1 ? 'signatur' : 'signaturer'}</p>{sharesSenderEmail && <p className="muted text-small">Du och mottagaren signerar var för sig, även när ni använder samma e-postadress.</p>}</div><div className="method-note"><span className="mini-check">✓</span><span>Ritad signatur</span></div><div className="actions between divider"><button className="button secondary" type="button" disabled={busy} onClick={() => { setStep(0); setError(''); }}>Tillbaka</button><button className="button" disabled={busy || preparing || !prepared}>{busy ? 'Skapar dokument…' : includeMe ? 'Skapa och signera' : 'Skicka för signering'}</button></div></form>}
      {step === 2 && created && (creationError ? <div className="stack">
        ...
        <div className="inline"><span className="circle medium">✓</span><div><h2>{created.document.status === 'completed' ? 'Dokumentet är färdigsignerat' : senderRecipient?.signedAt ? 'Din signatur är klar' : 'Redo att signeras'}</h2><p className="muted text-small">{created.document.status === 'completed' ? 'Alla parter har signerat. Den signerade PDF-filen finns på dokumentsidan.' : 'Dela den personliga länken med varje mottagare.'}</p></div></div>
        ...
        {sharingLinks.length > 0 && <><div className="stack small-gap">{sharingLinks.map(link => <div className="share-card" key={link.recipientId}><div className="inline"><Avatar name={link.name} /><div><strong>{link.name}</strong><p className="muted text-small">{created.document.recipients.find(r => r.id === link.recipientId)?.email}</p></div></div><CopyLink url={link.url} /></div>)}</div><p className="muted text-small">Spara länkarna nu. Varje länk ger tillgång till en mottagares signering. Inga e-postmeddelanden skickas automatiskt.</p></>}
...
export function Events({ doc, certificate = false }: { doc: SigningDocument; certificate?: boolean }) {
  return <div className={`timeline${certificate ? ' certificate-timeline' : ''}`}>{(doc.events ?? []).map(event => <div className="event" key={event.sequence}>{certificate && <div className="event-date"><strong>{date(event.at)}</strong><span className="mono muted">{new Date(event.at).toLocaleTimeString('sv-SE')}</span></div>}<div className="event-line"><i /><span /></div><div className="event-body"><p>{eventLabel(event, doc)}</p>{!certificate && <p className="event-meta mono">{dateTime(event.at)}</p>}{typeof event.data?.ip === 'string' && <p className="event-meta mono">IP {event.data.ip}</p>}{certificate && <p className="event-hash mono">{event.hash}</p>}</div></div>)}</div>;
}

export function DocumentDetail({ id, user, onBack, onCertificate }: { id: string; user: User; onBack: () => void; onCertificate: (doc: SigningDocument) => void }) {
  ...
  return <div><button className="text-button" onClick={onBack}>← Dokument</button><div className="detail-heading"><div className="stack small-gap"><Status doc={doc} /><h1>{doc.title}</h1><div className="document-meta"><span className="mono">{doc.id}</span><span>·</span><span>{pages(doc.pages)}</span><span>·</span><span>Skapad {date(doc.createdAt)}</span></div></div><div className="actions">{doc.status === 'pending' && ownRecipient && <button className="button" disabled={Boolean(busy)} onClick={() => void action('sign', async () => {
...
export function Certificate({ doc, onBack, onVerify }: { doc: SigningDocument; onBack: () => void; onVerify: () => void }) {
  ...
  return <div className="certificate-screen"><div className="actions between certificate-toolbar no-print"><button className="text-button" onClick={onBack}>← Tillbaka</button><div className="actions"><button className="button secondary small" onClick={() => { setError(''); void download(`/api/documents/${doc.id}/evidence`, `${doc.id}_verifikat.json`).catch(error => setError(message(error))); }}>Hämta bevisdata</button><button className="button secondary small" onClick={() => window.print()}>Skriv ut / spara PDF</button></div></div><ErrorBox error={error} /><article className="certificate"><div className="certificate-rule" /><div className="certificate-title"><div><span className="mono muted text-small">signhere</span><h1>Verifikat</h1></div><div className="stamp"><div><strong>SIGNHERE</strong><i /><b>{doc.status === 'completed' ? 'SLUTFÖRT' : doc.status === 'cancelled' ? 'AVBRUTET' : 'PÅGÅR'}</b><span className="mono">{doc.completedAt ? new Date(doc.completedAt).toLocaleDateString('sv-SE') : '—'}</span></div></div></div><div className="certificate-dots" />
      <h2>Dokument</h2><dl className="facts">{facts.map(([label, value]) => <div key={label}><dt>{label}</dt><dd className={label.startsWith('SHA') || label === 'Dokument-ID' ? 'mono' : ''}>{value}</dd></div>)}</dl>
