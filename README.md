...
- Block-based document editor (preview) at `/editor/…`. It has a cover, parties, pricing packages with VAT, rich text with dynamic `@` fields, image, terms and signature blocks, plus theming and a desktop/mobile preview. Drafts are saved in the browser for now. *Skicka* renders the draft to a PDF in the browser and continues in the normal upload flow with its title and signers filled in; *Använd som bilaga* does the same for a bilaga. The editor's signing deadline, reminder and decline settings are not applied yet.
...
Personal links are credentials. Signing consumes the ability to change that recipient's approval, while receipt access continues while the document is pending or finalizing, and until at least 30 days after completion; this is not literal destruction of the token after one use. Names are claims; a drawing and possession of a link do not establish verified identity. Network metadata describes the observed connection. Private JSON/ZIP evidence includes personal request details and should be shared deliberately.
