...
- Optional e-mail (`SMTP_URL`, `SMTP_FROM`): personal signing links are sent when a document or bilaga is created, and every signer gets a receipt link when it is fully signed. Without SMTP, links are shared manually as before.
...
3. **Environment variables:** set three under *Environment Variables*, before the first deploy:
   ...
   Optional: set `SMTP_URL` (for example `smtps://user:password@smtp.example.com:465`) and `SMTP_FROM` (for example `signhere <sign@example.com>`) to e-mail signing links and receipts. Mark `SMTP_URL` as a secret.
